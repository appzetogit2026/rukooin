...
const REQUIRED_DOCS_PG = ["Rent Agreement", "Owner ID Proof"];
const OPTIONAL_DOCS_PG = ["Local Body Registration"];

const PG_AMENITIES = ["WiFi", "Food", "Laundry", "Housekeeping", "Security"];
...
const AddPGWizard = () => {
  ...
  const [propertyForm, setPropertyForm] = useState({
    ...
    cancellationPolicy: 'No refund after move-in',
    houseRules: [],
    documents: [...REQUIRED_DOCS_PG, ...OPTIONAL_DOCS_PG].map(name => ({ name, fileUrl: '' }))
  });
  ...
  const [roomTypes, setRoomTypes] = useState([
    {
      ...
      extraChildPrice: 0,
      images: ['', '', '', ''],
      amenities: ['AC', 'Attached Washroom', 'Cupboard'],
      isActive: true
    }
  ]);
    ...
    const loadForEdit = async () => {
      ...
      try {
        ...
        setPropertyForm({
          ...
          pgType: prop.pgType || 'girls',
          amenities: prop.amenities || [],
          checkInTime: prop.checkInTime || '11:00 AM',
          checkOutTime: prop.checkOutTime || '10:00 AM',
          cancellationPolicy: prop.cancellationPolicy || 'No refund after move-in',
          houseRules: prop.houseRules || [],
          documents: docs.length
            ? docs.map(d => ({ name: d.name, fileUrl: d.fileUrl || '' }))
            : [...REQUIRED_DOCS_PG, ...OPTIONAL_DOCS_PG].map(name => ({ name, fileUrl: '' }))
        });
        if (rts.length) {
          setRoomTypes(
            rts.map(rt => ({
              ...
              totalInventory: rt.totalInventory ?? 5,
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          {step === 1 && (
            <>
              <div className="flex items-center gap-3 mb-4">
                ...
                <h2 className="text-lg font-bold">PG — Property Details + Documents</h2>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                <div className="flex items-center gap-3">
                  <span className="text-xs font-semibold text-gray-600">PG Type</span>
                  <select
                    className="input"
                ...
                <div className="grid grid-cols-1 gap-2 bg-gray-50 p-3 rounded-xl border border-dashed border-gray-300">
                  <h4 className="text-sm font-bold text-gray-700 mb-1">Documents</h4>
                  {propertyForm.documents.map((d, i) => (
                    <div key={i} className="flex items-center gap-2">
                      <FileText size={16} className="text-gray-500" />
                      <span className="text-sm w-44 truncate" title={d.name}>{d.name}</span>
                      <input
            ...
            <>
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <h2 className="text-lg font-bold">PG — Bed Sharing Types</h2>
                </div>
                <button
                  type="button"
                ...
                {roomTypes.map((rt, index) => (
                  <div key={rt.id} className="p-4 border border-gray-200 rounded-xl bg-gray-50/50 relative">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <input
                        className="input"
                        placeholder="Name (e.g. Single Sharing, Double Sharing)"
                        value={rt.name}
          ...
          {step === 3 && (
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <h3 className="text-2xl font-bold mb-2 text-gray-800">PG setup complete</h3>
              <p className="text-gray-500 text-center max-w-xs mb-6">
                PG property and bed-sharing inventories have been submitted. Admin verification ke baad property live ho jayegi.
              </p>
              <button
