...
const REQUIRED_DOCS_RESORT = ["Trade License", "GST Certificate", "FSSAI License", "Fire Safety Certificate"];
...
const AddResortWizard = () => {
  ...
  const [propertyForm, setPropertyForm] = useState({
    ...
    cancellationPolicy: '',
    houseRules: [],
    documents: REQUIRED_DOCS_RESORT.map(name => ({ name, fileUrl: '' }))
  });
    ...
    const loadForEdit = async () => {
      ...
      try {
        ...
        setPropertyForm({
          ...
          checkInTime: prop.checkInTime || '3:00 PM',
          ...
          documents: docs.length
            ...
            : REQUIRED_DOCS_RESORT.map(name => ({ name, fileUrl: '' }))
        });
        if (rts.length) {
          setRoomTypes(
            rts.map(rt => ({
  ...
  const onSubmitProperty = async () => {
    ...
    try {
      ...
      setError(e?.message || 'Failed to create resort');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          {step === 1 && (
            <>
              <div className="flex items-center gap-3 mb-4">
                ...
                <h2 className="text-lg font-bold">Resort — Property Details + Documents</h2>
              </div>
              ...
              <div className="grid grid-cols-1 gap-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <select
                    className="input"
                    value={propertyForm.resortType}
                    onChange={e => updatePropertyForm('resortType', e.target.value)}
                  >
                    <option value="beach">Beach Resort</option>
                    <option value="hill">Hill Resort</option>
                    <option value="jungle">Jungle Resort</option>
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          updatePropertyForm(
                            'activities',
                            has
                              ? propertyForm.activities.filter(x => x !== act)
                              : [...propertyForm.activities, act]
                          );
                        }}
                        className={`px-3 py-1 rounded-full text-xs font-medium border ${propertyForm.activities.includes(act)
                ...
                <div className="space-y-2">
                  <h3 className="text-sm font-semibold">Resort Amenities</h3>
                  <div className="flex flex-wrap gap-2">
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          updatePropertyForm(
                            'amenities',
                            has
                              ? propertyForm.amenities.filter(x => x !== am)
                              : [...propertyForm.amenities, am]
                          );
                        }}
                ...
                <div className="grid grid-cols-1 gap-2">
                  {propertyForm.documents.map((d, i) => (
                    <div key={i} className="flex items-center gap-2">
                      ...
                      <span className="text-sm w-52">{d.name}</span>
                      <input
                        className="input flex-1"
              ...
              >
                {loading ? 'Creating...' : 'Create Resort Property'}
              </button>
            </>
          )}
          ...
          {step === 2 && (
            <>
              <div className="flex items-center gap-3 mb-4">
                ...
                <h2 className="text-lg font-bold">Resort — Room / Cottage Types</h2>
              </div>
              {error && <div className="text-red-600 text-sm mb-3">{error}</div>}
                  ...
                  <div key={rt.id} className="border border-gray-200 rounded-xl p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      <input
                        ...
                        placeholder="Unit Name (e.g., Deluxe Cottage)"
                        value={rt.name}
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <input
                        className="input"
                        placeholder="Total Inventory (units)"
          ...
          {step === 3 && (
            <div className="flex flex-col items-center justify-center py-8">
              ...
              <h3 className="text-xl font-bold mb-1">Resort setup complete</h3>
