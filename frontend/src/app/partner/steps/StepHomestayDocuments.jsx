...
const StepHomestayDocuments = () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-[#003836]">Documents & Verification</h2>
        <p className="text-sm text-gray-500">Upload required documents for verification</p>
      </div>

      <div className="bg-red-50 p-4 rounded-lg text-red-800 text-sm mb-4 border border-red-200">
        ⚠️ <strong>Important:</strong> All documents must be valid, clear, and legible. Your listing will be approved only after verification.
      </div>

      <div className="space-y-3">
        <h3 className="font-bold text-md text-gray-700 mt-4">Mandatory Documents</h3>
        {renderDocUpload('hostIdProof', '📄 Host Aadhaar / ID Proof', true)}
        {renderDocUpload('ownershipProof', '🏠 Property Ownership Proof / Rent Agreement', true)}
        {renderDocUpload('localRegistration', '📜 Local Authority Registration (if required)', false)}
      </div>

      <div className="bg-blue-50 border border-blue-100 p-4 rounded-lg">
        <p className="text-sm text-blue-800">
          <strong>ℹ️ Verification:</strong> Our team will verify documents within 48-72 hours. You'll be notified via email.
        </p>
      </div>
    </div>
  );
...
export default StepHomestayDocuments;
