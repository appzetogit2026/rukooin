...
const StepTerms = () => {
    ...
    return (
        <div ref={containerRef} className="pb-24 pt-2 px-1 h-full flex flex-col">
            <h3 className="text-xl font-bold mb-4 text-gray-900 px-1 flex items-center gap-2">
            ...
            <div className="anim-item px-1">
                <button
                    ...
                    className="flex items-start gap-4 p-4 rounded-2xl border transition-all w-full text-left bg-white shadow-sm hover:border-black group"
                    style={{ borderColor: formData.termsAccepted ? 'black' : '#e5e7eb' }}
                >
                    <div className={`mt-0.5 w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all ${formData.termsAccepted ? 'bg-black border-black text-white' : 'border-gray-300 bg-white text-transparent'
                        }`}>
                        <Check size={14} strokeWidth={4} />
                    </div>
                    <div>
                        <span className="block font-bold text-gray-900 text-sm">I accept the Terms & Conditions</span>
